/**
 * frontend/src/services/dashboardService.ts - Serviço de dashboard para frontend
 */
...
export async function getInadimplencia(meses = 6) {
  const response = await api.get(`/dashboard/inadimplencia?meses=${meses}`);
  return response.data;
}
